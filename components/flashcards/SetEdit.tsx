...
export default function SetEdit({ set }: { set: FlashcardSet }) {
  ...
  const { mutate: deleteSet, isLoading: isSetDeleting} = useMutation({
    ...
    onError: () => {
      toast({
        title: "Uh oh, something went wrong!",
        description: <p>There was an error deleting the flashcard set.</p>,
        variant: "destructive",
        action: (
          <ToastAction altText="Try again" onClick={() => deleteSet()}>
            Try again
          </ToastAction>
        ),
      })
        ...
        <Form {...form}>
          <form onSubmit={form.handleSubmit(onSubmit)} className="space-y-4">
            ...
            <Button className="w-full" type="submit">
              Submit{" "}
              {isSetEditing && <Loader2 className="h-4 w-4 ml-2 animate-spin" />}
            </Button>
          </form>
        </Form>
      </DialogContent>
